...
interface GroupedSetbacks {
 [developmentType: string]: PRP_K7_SetbackResult[];
}

export class PRPK7DatabaseClient {
 private pool: Pool;
 ...
 */
 async getZoneSetbacksGroupedByDevType(zone: string): Promise<GroupedSetbacks> {
 ...
 const grouped: GroupedSetbacks = {};

 for (const row of result.rows) {
 const devType = row.development_type;
 // More flexible boundary type extraction
 let boundaryType = row.context;
 if (boundaryType.includes('setback_')) {
 boundaryType = boundaryType.replace('setback_', '');
 } else if (boundaryType.includes('setback')) {
 boundaryType = boundaryType.replace(/.*setback[_\s]*/i, '');
 }
 if (!boundaryType || boundaryType === row.context) {
 boundaryType = 'general';
 }
 
 const setbackResult: PRP_K7_SetbackResult = {
 id: row.id,
 zone: row.zone,
 ...
 */
 async getPermittedDevelopmentTypes(zone: string): Promise<string[]> {
 const client = await this.pool.connect();
 try {
 const query = `
 ...
 */
 async getZoneSetbacksFlat(zone: string): Promise<any[]> {
 ...
 */
 private getAuthorityLevel(documentId: string): string {
 ...
 */
 private getPrecedence(documentId: string): number {
 ...
 */
 async getZoneStats(zone: string): Promise<any> {
 const client = await this.pool.connect();
 try {
